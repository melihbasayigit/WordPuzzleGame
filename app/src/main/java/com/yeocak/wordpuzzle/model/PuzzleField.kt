...
class PuzzleField(val columnNumber: Int, val rowNumber: Int) {
	...
	fun setBounds(viewWidth: Int, viewHeight: Int) {
		...
		// Set first row of screen
		firstRowOfScreen = getLetterTableLocation(PointF(0f, 0f)).first
	}
	...
	fun getLetterTableLocation(point: PointF): Pair<Int, Int> {
		val originPoint = PointF(point.x + horizontalPadding, -point.y + verticalPadding + height)

		val singleBoxSize = letterSize + LETTER_SPACING
		val rowColumnZeroPoint = PointF(fieldCoordinates.bottom, fieldCoordinates.left)
		val horizontalDistance = floor(originPoint.x - rowColumnZeroPoint.x)
		val verticalDistance = floor(originPoint.y - rowColumnZeroPoint.y)

		val column = (horizontalDistance) / singleBoxSize
		val row = (verticalDistance) / singleBoxSize
